...
class FileStorageUtility {
  ...
  static Future<File?> setString({
    ...
    try {
      return await fileStorage.writeAsString(text);
    } catch(e) {
      log('FileRepositoryService: set $e');
    }
  ...
  static Future<File?> get(final String? filePath, {
    final bool isDocumentDirectory = false,
    final bool isTemporaryDirectory = false,
  }) async {
    final String directoryPath;

    if (isDocumentDirectory) {
      directoryPath = (await _instanceDocumentDirectory).path;
    } else if (isTemporaryDirectory) {
      directoryPath = (await _instanceTemporaryDirectory).path;
    } else {
      directoryPath = '';
    }
    ...
    final file = File('${directoryPath.isEmpty ? '' : '$directoryPath/'}$filePath');
